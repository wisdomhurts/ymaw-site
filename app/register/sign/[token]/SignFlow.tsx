...
import { Agreement, Signature, StepHead } from "@/components/register/fields";
...
export default function SignFlow({ token }: { token: string }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const r = await fetch("/api/sign", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, initials: inits, signature: sig }) });
  ...
  return (
    <div>
      ...
      <div className="mt-6"><Signature label="Your signature" value={sig} onChange={setSig} hint="Type your full name. That's your word." error={errs.sig} /></div>
